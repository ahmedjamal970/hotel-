...
$_SESSION['departure'] =date_format(date_create($_POST['departure']),"Y-m-d");
  ...
  $query = "SELECT * FROM `tblroom` r ,`tblaccomodation` a WHERE r.`ACCOMID`=a.`ACCOMID`   AND `NUMPERSON` = " . $_POST['person'] ;  
    ...
    $hotelId = $_GET['roomNO']; 
    echo $hotelId;
     $query = "SELECT * FROM `tblroom` r ,`tblaccomodation` a WHERE r.`ACCOMID`=a.`ACCOMID`" . " AND  `HOTELID` = " . $hotelId; 
  }
                ...
                ?>
                 ...
                 <input type="hidden" name="ROOMPRICE" value="<?php echo $result->PRICE ;?>">
                  <input type="hidden" name="ROOMID" value="<?php echo $result->ROOMID ;?>">
                      ...
                      <div class="card">
                        <img class="card-img-top"  src="<?php echo WEB_ROOT .'admin/mod_room/'.$result->ROOMIMAGE; ?>" alt="Room image description" height="250px" width="100px">
                        <div class="card-body">
